    ...
    try:
        ...
        if async_mode:
            ...
            def run_export_async():
                ...
                try:
                    result = do_export()
                    add_log(f"export 完了 (async): success={result.get('success')}")
                    with job_state_lock:
                        export_result_store = {
                            "result": result,
...
def api_export_result():
    """CSV出力の結果を取得（非同期モードのポーリング用）

    GASから定期的に呼び出し、エクスポートが完了したかを確認する。
    """
    with job_state_lock:
    ...
    return jsonify({
        "success": False,
        "status": "no_result",
        "message": "エクスポート結果がありません。先に /api/export を呼び出してください。",
    })
